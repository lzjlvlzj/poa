...
function checkSelectedData( bAllowSelectMultiple ){
	var count = jQuery("#TableData").find("input[type=checkbox]:checked").size();
	if(count == 0){
		alert("请选择要操作的数据");
		return false;	
	}
...
function getSelectedDataValues(){
	var jQueryselectedData = jQuery("#TableData").find("input[type=checkbox]:checked");
	var valueArray = new Array();
	jQueryselectedData.each(function(){
...
function content_onclick( oTr ) {
	...
	jQuery("input[name=selectAll]").attr("checked", false);
	jQuery("input[name=dataItem]").attr("checked", false);
	
	// 还原上次选中的checkbox行的class
	if(jQuerylastCheckedTr != null){ 
		jQuerylastCheckedTr.attr("class", lastCheckedTrClassName);
	}
	// 再记录这次选中行的信息（以备下次还原用）
	jQuerylastCheckedTr = jQuery(oTr);
	lastCheckedTrClassName = jQuerylastCheckedTr.attr("class");
	
	// 再选中当前checkbox
    var jQuerycb = jQuery(oTr).find("input[type=checkbox]");
	jQuerycb.attr("checked", true);
	
	// 改变当前选中行的样式（高亮）
	jQuerylastCheckedTr.attr("class", "selectLine");
}
...
/**
* 输入框内的提示文字显示与设置
* 注：代码jQuery(inputElement).attr("showText")处，没有使用inputElement.showText。因为后者在firefox下不能运行，所以采用jQuery框架处理。
*/
//******************************************************
function inputAreaClick(inputElement, showText) {
	if(showText == null){
		showText = jQuery(inputElement).attr("showText");
	}
...
//	jQuery("input[type=text][showText]").each(function(){
//		this.value = jQuery(this).attr("showText");
//	});

	// 设置所有的文本框在选中时，选中文本框中的文本
	jQuery("input[type=text]").each(function(){;
		// this.onFocus = function() { this.select(); }; // 用这行代码不起作用!?
		jQuery(this).focus(function(){ // 改用jQuery实现
			this.select();
		});
	});
